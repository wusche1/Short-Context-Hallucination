...
# %%
class Conversation():
    def __init__(self, model: Optional[AutoModelForCausalLM] = None, tokenizer: Optional[AutoTokenizer] = None, system_prompt = "You are a helpful honest assistant"):  
    ...
    def prompt_llama(self, prompt: str, ignore_until = 0) -> str:
        ...
        tokens = self.tokenizer.apply_chat_template(self.messages, return_tensors="pt", tokenize=True, add_generation_prompt=True).to(device)
        n_tokens = tokens.shape[1]

        mask = t.ones_like(tokens)
        if ignore_until != 0:
            tokens_to_be_ignored = self.tokenizer.apply_chat_template(self.messages[:ignore_until], return_tensors="pt", tokenize=True, add_generation_prompt=True)
        ...
        response = self.tokenizer.decode(out["sequences"][0,n_tokens:], skip_special_tokens=True)
